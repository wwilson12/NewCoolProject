...
function searchDragDictionary(){
    ...
    if (dragWords == 'purse first' || dragWords == 'purse' || dragWords == 'purse purse' || dragWords == 'walk into the room'){ //if string is equal to any of these
        ...
        $(document.body).append('<div class="deftxt">Purse first (adj): A way in which one walks into the room to demonstrate fierceness. It is a known fact that a lady do carry an evening bag.</div>');
    }
    else if (dragWords == 'sickening' || dragWords == 'lets get sickening' || dragWords == 'sickening'){
        $( '.deftxt' ).remove();
        $('body').addClass('sickening');
        $(document.body).append('<div class="deftxt">Sickening (adj): To be so fabulous, you make all others sick with jealousy.</div>');
    }
    else if (dragWords == 'the shade' || dragWords == 'shade' || dragWords == 'shadey'){
        $( '.deftxt' ).remove();
        $('body').addClass('theshade');
        $(document.body).append('<div class="deftxt">Shade (noun): Disrespect towards another person. Dont be throwing no shade.</div>');
    }
    else if (dragWords == 'yes god' || dragWords == 'yas god' || dragWords == 'yes gawd'|| dragWords == 'yas gawd'){
        $( '.deftxt' ).remove();
        $('body').addClass('yesgod');
        $(document.body).append('<div class="deftxt">Yes god (expression): To be so enthralled with something/someone you must praise the heavens for their existance.</div>');
    }
    else if (dragWords == 'back rolls' || dragWords == 'backrolls' || dragWords == 'rolls'|| dragWords == 'back'){
        $( '.deftxt' ).remove();
        $('body').addClass('backrolls');
        $(document.body).append('<div class="deftxt">Backrolls (noun): Skin on ones back that is pushed together due to the tightness of a garment to resemble rolling hills.</div>');
    }
    else if (dragWords == 'eleganza' || dragWords == 'eleganza extravaganza' || dragWords == 'extravaganze'){
        $( '.deftxt' ).remove();
        $('body').addClass('eleganza');
        $(document.body).append('<div class="deftxt">Eleganza (noun): A combination of elegnace and extravangance. Eg: They were gagging for her eleganza.</div>');
    }
    else if (dragWords == 'fish' || dragWords == 'tuna' || dragWords == 'fishy' || dragWords == 'tuna on a platter'){
        $( '.deftxt' ).remove();
        $('body').addClass('fish');
        $(document.body).append('<div class="deftxt">Fish (noun, adj): To display ones self in an extremely feminine way. Honey I am serving up fish, tuna on a platter.</div>');
    }
    else if (dragWords == 'gagging' || dragWords == 'gag' || dragWords == 'girl i am gagging' || dragWords == 'gagg'){
        $( '.deftxt' ).remove();
        $('body').addClass('gagging');
        $(document.body).append('<div class="deftxt">Gag (verb): To be blown away, either because something is so ridiculous, funny, or incredible. Eg: I am gaggin for your look!</div>');
    }
    else if (dragWords == 'huntey' || dragWords == 'hunty' || dragWords == 'hunties'){
        $( '.deftxt' ).remove();
        $('body').addClass('huntey');
        $(document.body).append('<div class="deftxt">Hunty (noun): A combination of honey and a less endearing C word. A term of affection.</div>');
    }
    else if (dragWords == 'lil pound cake' || dragWords == 'little pund cake' || dragWords == 'pound cake'){
        $( '.deftxt' ).remove();
        $('body').addClass('lilpoundcake');
        $(document.body).append('<div class="deftxt">Lil Pound Cake (noun): A pageant princess with a sharp attitude. Enjoys her go-go juice and harrassing her stepfather.</div>');
    }
    else if (dragWords == 'ornacia'){
        $( '.deftxt' ).remove();
        $('body').addClass('ornacia');
        $(document.body).append('<div class="deftxt">Ornacia (noun): A head with a lot of attitude. Perhaps one of the most underrated Drag Race contestants.</div>');
    }
    else if (dragWords == 'sashay away' || dragWords == 'sashay'){
        $( '.deftxt' ).remove();
        $('body').addClass('sashay');
        $(document.body).append('<div class="deftxt">Sashay Away (verb):To exit the competition in style and with class.</div>');
    }
    else if (dragWords == 'sashay away' || dragWords == 'sashay'){
        $( '.deftxt' ).remove();
        $('body').addClass('sashay');
        ...
        $(document.body).append('<div class="deftxt">She Aldready Done Has Herses (?): Something RuPaul says, yet no one understands.</div>');
    }
    else if (dragWords == 'tea' || dragWords == 'no tea no shade' || dragWords == 'all tea all shade'|| dragWords == 't'){
        $( '.deftxt' ).remove();
        $('body').addClass('tea');
        $(document.body).append('<div class="deftxt">Tea (noun): The truth! Eg: She seems interesting, what is the tea?</div>');
    }
    else if (dragWords == 'pop' || dragWords == 'tongue pop' || dragWords == 'tongue'|| dragWords == 'popping'){
        $( '.deftxt' ).remove();
        $('body').addClass('tonguepop');
        $(document.body).append('<div class="deftxt">Tongue pop (noun, verb): An action one does in which one pops their tongue from the top of their mouth. Used for emphasis and drama.</div>');
    }
    else if (dragWords == 'turned' || dragWords == 'turned it' || dragWords == 'turn'|| dragWords == 'turn it'){
        $( '.deftxt' ).remove();
        $('body').addClass('turnedit');
        $(document.body).append('<div class="deftxt">Turn it (verb): An action in which one takes something that is just ok and makes int fabulous.</div>');
    }
